...
class ScreenshotUtil:
    ...
    def take_screenshot(driver, page_name):
        try:
            ...
            logger.error(
                f"Failed to take screenshot for '{page_name}': {error}"
            )
            return None
    ...
    def take_full_page_screenshot(driver, page_name):
        try:
            ...
            allure.attach.file(
                filepath,
                name=f"{page_name}_fullpage",
                attachment_type=allure.attachment_type.PNG
            )

            logger.info(
                f"Full page screenshot saved: {filepath}"
            )
            ...
            logger.error(
                f"Failed to take full-page screenshot for '{page_name}': {error}"
            )
            return None